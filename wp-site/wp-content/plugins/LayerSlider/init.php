...
if(is_array($slides)) {
    ...
    $data .= '<script type="text/javascript">' . NL;
		...
		$data .= "var is_logged_offset = parseInt($is_logged) ? 32 : 0;" . NL;
		$data .= "var windowHeight = parseInt('$height') ? parseInt('$height') + 'px' : (lsjQuery(window).height() - is_logged_offset) + 'px';" . NL;
		$data .= "var touch = Modernizr.touch;" . NL;
		$data .= "windowHeight = !touch ? windowHeight : '650px' || '600px';" . NL;
