...
class TMM_Custom_Sidebars {
	...
	public static function get_categories_select($selected = '', $name = '', $id = '') {
		$args = array(
			'show_option_none' => __('No categories', 'accio'),
			'hide_empty' => 0,
			'echo' => 0,
			'selected' => $selected,
			'hierarchical' => 0,
	...
	public static function get_pages_select($selected = '', $name = '', $id = '') {
		$args = array(
			'show_option_none' => __('No pages', 'accio'),
			'selected' => $selected,
			'echo' => 0,
			'name' => $name,
			'id' => $id
		...
		if ($show_default_sidebar) {
			if (function_exists('dynamic_sidebar') AND dynamic_sidebar('Thememakers Default Sidebar')):else:
				...
				<div class="widget widget_categories">
					<h3 class="widget-title"><?php _e('Categories', 'accio') ?></h3>
					<ul class="categories">
						<?php wp_list_categories('sort_column=name&optioncount=1&hierarchical=0&title_li=0'); ?>
					</ul>
				</div>
				<div class="widget widget_calendar">
					<h3 class="widget-title"><?php _e('Calendar', 'accio') ?></h3>
					<?php get_calendar(); ?>
				</div>

				<div class="widget widget_meta">
					<h3 class="widget-title"><?php _e('Meta', 'accio') ?></h3>
					<ul>
						<?php wp_register(); ?>
						<li>
							<?php wp_loginout(); ?>
